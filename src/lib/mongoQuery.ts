...
export default function mongoQuery(q: string) {
  ...
  const queryArray = Object.entries(queryObject).map(([key, value]) => {
    ...
    if (Array.isArray(value)) {
      if (key.indexOf('.') !== -1) {
        const embedEntries = key.split('.');
        return {
          [embedEntries[0]]: { $elemMatch: { [embedEntries[1]]: { $gte: value[0], $lte: value[1] } } },
        };
      }
    ...
    if (key.indexOf('*') != -1) {
      ...
      if (key.indexOf('!') != -1) {
        return {
          [key.replace('!*', '')]: { $all: values.map((value) => ({ $elemMatch: { $ne: value } })) },
        };
      }
